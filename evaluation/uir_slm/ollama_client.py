...
class OllamaClient:
    ...
    def generate(self, prompt: str, system: str, options: dict) -> GenerationResult:
        payload = {"model": self.config["model"], "prompt": prompt, "system": system, "stream": False, "format": "json", "keep_alive": self.config.get("keep_alive", "30m"), "options": {"temperature": options["temperature"], "top_p": options["top_p"], "top_k": options["top_k"], "num_predict": options["max_new_tokens"], "repeat_penalty": options["repetition_penalty"], "seed": options["seed"]}}
