...
def rate(group: list[dict], key: str) -> float:
    ...
    if key == "latency_us":
        return statistics.fmean(row["latency"]["pipeline_total_us"] for row in group)
    return statistics.fmean(row["metrics"][key] for row in group)
...
def main() -> None:
    ...
    for mode, filename in (("det-repeat", "cross_run_variance.csv"), ("stochastic", "cross_seed_variance.csv")):
        ...
        for pipeline in sorted({key[0] for key in grouped}):
            for metric in metrics:
                ...
                output.append({"pipeline": pipeline, "metric": metric, "runs": len(values), "mean": statistics.fmean(values) if values else 0.0, "variance": statistics.pvariance(values) if values else 0.0, "minimum": min(values, default=0.0), "maximum": max(values, default=0.0)})
