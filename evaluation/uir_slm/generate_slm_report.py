...
def main() -> None:
    ...
    core_summary = {"cases": len(core), "structural_match": sum(row["exact_structural_match"] for row in core) / len(core), "semantic_match": sum(row["semantic_match"] for row in core) / len(core), "policy_accuracy": sum(row["expected_policy_decision"] == row["actual_policy_decision"] for row in core) / len(core), "outcome_accuracy": sum(row["correct"] for row in core) / len(core)}
    failure_counts = Counter(row["error_type"] for row in failures)
    sections = [
        "# Phase UIR-2 SCI Evaluation Report",
        "## 1. Summary\n\nA frozen external test, real SEC fact registry, real local Phi-3.5 renderer, B0–B5 baselines, groundedness, numeric, adversarial, generalization, robustness, statistics, and runtime evidence were evaluated without changing the frozen set after hashing.",
        ...
        f"## 3. Frozen Test Dataset and Hash\n\nCases: {frozen['case_count']}; SHA-256: `{frozen['dataset_sha256']}`; human review status: `{frozen.get('manual_review_status', 'not_recorded')}`.",
        ...
        "## 13. Runtime Results\n\n" + table(latency, ["pipeline", "cases", "mean_us", "p50_us", "p95_us", "prompt_eval_mean_us", "generation_mean_us", "validator_mean_us"]),
        ...
        "## 17. Limitations\n\nThe frozen set was programmatically curated but not manually reviewed, only one local SLM and one host were evaluated in P0, and SEC values reflect the snapshot date rather than a timeless ground truth.",
        "## 18. Recommended Paper Claims\n\nClaims should be limited to the measured model, frozen data, host, and confidence tests. Do not generalize Phi-3.5 results to all SLMs or describe prompt constraints as enforcement.",
