...
def main() -> None:
    ...
    phase3d = workspace / "evaluation/uir_phase3d"
    audit_ingest = (phase3d / "ingest_actual_ai_reviews.py").read_text(encoding="utf-8")
    require("actual_model_generation" in audit_ingest and "rule-based/script annotation is not admissible" in audit_ingest
            and "raw_response_sha256" in audit_ingest,
            "ARCH-UIR-3D-001", "actual-model audit ingestion admits script judgments or lacks provenance")
    passed.append("ARCH-UIR-3D-001")
    baseline_text = (workspace / "evaluation/uir_slm/baselines.py").read_text(encoding="utf-8")
    require("FACT_REF_RESPONSE_SCHEMA" in baseline_text and "fact_reference_prompt" in baseline_text
            and "Never reproduce numeric values" in baseline_text,
            "ARCH-UIR-3D-002", "SEC compact fact-reference protocol missing")
    passed.append("ARCH-UIR-3D-002")
    config = json.loads((workspace / "evaluation/uir_slm/model_config/phi35_ollama.json").read_text())
    require(config["deterministic"]["max_new_tokens"] >= 384,
            "ARCH-UIR-3D-003", "structured output generation budget below Phase3D candidate")
    passed.append("ARCH-UIR-3D-003")
    b6_tests = (phase3d / "tests/test_b6_integration.py").read_text(encoding="utf-8")
    require(all(name in b6_tests for name in ("test_b6_keeps_supported_subset", "test_b6_removes_unsupported_subset",
            "test_b6_partial_answer_state", "test_b6_rejects_when_no_supported_claim", "test_b6_unsupported_acceptance_zero")),
            "ARCH-UIR-3D-004", "required B6 integration tests missing")
    passed.append("ARCH-UIR-3D-004")
    phase3d_gate = (phase3d / "publication_gate_phase3d.py").read_text(encoding="utf-8")
    require("actual_multi_model_audit_complete" in phase3d_gate and "READY_FOR_MANUSCRIPT_DRAFT" in phase3d_gate,
            "ARCH-UIR-3D-005", "Phase3D gate does not require actual multi-model audit")
    passed.append("ARCH-UIR-3D-005")
