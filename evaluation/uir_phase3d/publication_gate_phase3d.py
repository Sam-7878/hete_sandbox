...
def main():
    audit=load(OUT/"actual_ai_audit_summary.json");campaign=load(OUT/"campaign_summary.json");budget=load(OUT/"generation_budget_validation.json");b6=load(OUT/"b6_filtering_summary.json");tests=load(OUT/"b6_integration_tests.json");frozen=load(P3B/"FROZEN_TEST_V2_MANIFEST.json")
    ...
    checks={"actual_multi_model_audit_complete":audit.get("status")=="complete" and audit.get("shared_three_model_cases",0)>=400,
            "model_review_provenance_recorded":bool(audit.get("review_file_sha256")),"agreement_statistics_valid":(OUT/"actual_ai_agreement.csv").exists() and audit.get("unresolved")==0,
