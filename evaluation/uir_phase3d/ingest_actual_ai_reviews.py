...
def main():
    ...
    if len(shared)<400 or covered!=case_ids:raise ValueError(f"coverage insufficient: shared={len(shared)}, any={len(covered)}/{len(case_ids)}")
    ...
    for field in FIELDS:
        ...
        for i,cid in enumerate(ordered):
            ...
            if len(set(vals))>1:
                ...
                disagreements.append({"case_id":cid,"field":field,"r1_judgment":vals[0],"r2_judgment":vals[1],"r3_judgment":vals[2],"majority":majority,
    ...
    dfields=["case_id","field","r1_judgment","r2_judgment","r3_judgment","majority","r1_rationale","r2_rationale","r3_rationale","status"]
    ...
    summary={"status":"complete","protocol":"triple_independent_actual_model_audit","shared_three_model_cases":len(shared),"any_model_cases":len(covered),
             "full_1200x3":all(len(x)==1200 for x in data.values()),"unresolved":sum(x["status"]=="unresolved" for x in disagreements),
