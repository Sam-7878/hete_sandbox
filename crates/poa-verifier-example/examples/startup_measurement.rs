...
use poa_protocol::{DeploymentMode, OsBackend, PolicyRepository, canonicalize, policy_digest, validate_value};
...
fn main() {
    for iteration in 0..20 {
        ...
        let base_value: Value = serde_json::from_str(include_str!("../../../protocol/base/hete.base.verifier.json")).unwrap();
        let child_value: Value = serde_json::from_str(include_str!("../../../protocol/examples/hete.verifier.payment.json")).unwrap();
        ...
        let started = Instant::now();
        let effective = PolicyRepository::new([base, child]).resolve("hete.verifier.payment").unwrap().policy;
        let t_inheritance = started.elapsed().as_micros();
        let started = Instant::now(); let _canonical = canonicalize(&effective).unwrap(); let t_canonicalize = started.elapsed().as_micros();
        let started = Instant::now(); let digest = policy_digest(&effective).unwrap(); let t_digest = started.elapsed().as_micros();

        let started = Instant::now();
        let listener = TcpListener::bind("127.0.0.1:0").unwrap();
        let mut enforcement = StartupEnforcement { listener_initialized: true, ..Default::default() };
        enforcement.enforce_after_listener(&NoOpDevelopmentBackend, &effective).unwrap();
        drop(listener);
