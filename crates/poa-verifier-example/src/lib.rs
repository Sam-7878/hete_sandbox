...
impl Verifier {
    pub fn new(policy: EffectivePolicy, request_schema: Value) -> anyhow::Result<Self> {
        jsonschema::validator_for(&request_schema).map_err(|e| anyhow::anyhow!("request schema: {e}"))?;
        let digest = policy_digest(&policy)?;
        Ok(Self { policy, digest, request_schema, domain_state: DomainState::default(), trust_state: TrustState::default() })
    }

    pub fn policy_digest(&self) -> &str { &self.digest }
    ...
    pub fn process_bytes(&mut self, bytes: &[u8]) -> ProcessingResult {
        ...
        let transition_id = serde_json::from_slice::<Value>(bytes).ok()
            .and_then(|v| v.pointer("/context/request_id").and_then(Value::as_str).map(str::to_owned))
    ...
    fn process_bytes_inner(&mut self, bytes: &[u8], actor_hint: &str) -> TransitionOutcome {
        if bytes.len() > self.policy.data_constraints.maximum_message_bytes as usize {
            return self.violation(actor_hint, RejectReason::InvalidInput("maximum_message_bytes exceeded".into()));
        }
        ...
        if nesting_depth(&value) > self.policy.data_constraints.maximum_nesting_depth {
            return self.violation(actor_hint, RejectReason::InvalidInput("maximum nesting depth exceeded".into()));
        }
        ...
        let operation = match self.policy.operations.iter().find(|o| o.name == request.operation) {
            Some(op) => op.clone(),
        ...
        for required in &operation.required_context {
            if !context_present(&request.context, required) {
                return self.violation(&request.actor, RejectReason::MissingContext(required.clone()));
            }
        }
        ...
        let mut hooks = PaymentHooks { operation: &operation, request: &request, state: &mut self.domain_state };
        execute_transition(&mut hooks, &descriptor)
    }

    fn violation(&mut self, actor: &str, reason: RejectReason) -> TransitionOutcome {
        let count = self.trust_state.violations.entry(actor.to_owned()).or_default();
        *count += 1;
    ...
    pub fn authorize_inbound(&self, source: IpAddr, port: u16, protocol: &str) -> bool {
        self.policy.network_policy.as_ref().is_none_or(|n| n.inbound.iter().any(|r| endpoint_matches(r, source, port, protocol)))
    }

    pub fn authorize_outbound(&self, destination: IpAddr, port: u16, protocol: &str) -> bool {
        self.policy.network_policy.as_ref().is_some_and(|n| n.outbound.iter().any(|r| endpoint_matches(r, destination, port, protocol)))
    }
}

fn endpoint_matches(rule: &EndpointRule, address: IpAddr, port: u16, protocol: &str) -> bool {
    rule.protocol == protocol && rule.port == port && rule.address.parse::<IpAddr>().is_ok_and(|a| a == address)
}
...
impl AacoHooks<String, String, RequestContext, String> for PaymentHooks<'_> {
    ...
    fn authorize(&self, descriptor: &TransitionDescriptor<String, String, RequestContext, String>) -> Result<(), RejectReason> {
        if self.operation.allowed_actors.contains(&descriptor.actor) { Ok(()) } else { Err(RejectReason::DisallowedActor) }
    }

    fn validate(&self, _: &TransitionDescriptor<String, String, RequestContext, String>) -> Result<(), RejectReason> {
        if self.request.payload.amount == 0 { Err(RejectReason::InvariantViolation("amount must be positive".into())) } else { Ok(()) }
    }

    fn mutate_candidate(&mut self, descriptor: &TransitionDescriptor<String, String, RequestContext, String>) -> Result<Self::Candidate, AbortReason> {
        if self.request.payload.inject_internal_error {
            return Err(AbortReason::InternalFailure("injected storage failure".into()));
        }
...
pub fn resolved_policy_canonical(policy: &EffectivePolicy) -> anyhow::Result<Vec<u8>> { Ok(canonicalize(policy)?) }
